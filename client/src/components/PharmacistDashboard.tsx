...
export function PharmacistDashboard() {
  ...
  if (!user || user.type !== 'pharmacist') return <Navigate to="/auth" replace />;
  ...
  return (
    <div>
      <header style={{ background: '#fff', borderBottom: '1px solid var(--gray-200)', padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ fontWeight: 'bold', fontSize: '1.25rem' }}>MediCare - Pharmacy</div>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100vh - 73px)' }}>
        <aside style={{ width: '260px', background: '#fff', borderRight: '1px solid var(--gray-200)', padding: '1.5rem 1rem' }}>
          <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            ...
            <div style={{ padding: '0.75rem 1rem', color: 'var(--gray-600)', cursor: 'pointer' }}>Prescriptions</div>
            <div style={{ padding: '0.75rem 1rem', color: 'var(--gray-600)', cursor: 'pointer' }}>Inventory</div>
            <div style={{ padding: '0.75rem 1rem', color: 'var(--gray-600)', cursor: 'pointer' }}>Sales</div>
        ...
        <main style={{ flex: 1, padding: '2rem', background: 'var(--gray-50)' }}>
          ...
          <p>Your pharmacist dashboard is ready.</p>
        </main>
      </div>
    </div>
  );
